...
#+DATE: 2022-01-17
#+DESCRIPTION: 通过 SSLKEYLOGFILE 环境变量导出 TLS 会话密钥,让 wireshark 解密 https 流量。含 windows / Linux / macOS 三种设置方式。
...
查看过滤结果 如下图所示
#+ATTR_HTML: :alt 上半是终端里 curl -LI https://google.com 的响应头,下半是 wireshark 用 tls and (http or http2) 过滤后解密出的 HTTP2 帧,选中的第 674 号包是 301 Moved Permanently,与 curl 的输出对应
